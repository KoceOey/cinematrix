...
func AuthenticateUser(next http.HandlerFunc, accessType string) http.HandlerFunc {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !isValidToken {
			sendResponse(w, 400, "Token tidak valid!, Mohon login user yang sesuai!")
		} else {
			next.ServeHTTP(w, r)
		}
	})
...
func getUserTokenData(r *http.Request) (int, string, string) {
	if cookie, err := r.Cookie(userTokenName); err == nil {
		jwtToken := cookie.Value
		accessClaims := &UserClaims{}
		parsedToken, err := jwt.ParseWithClaims(jwtToken,
		...
		if err == nil && parsedToken.Valid {
			return accessClaims.ID, accessClaims.Name, accessClaims.UserType
		}
	}
	return -1, "", ""
}
